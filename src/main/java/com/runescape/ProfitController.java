...
public class ProfitController implements Initializable {
    ...
    @FXML
    private TableColumn<Item, BigInteger> profitColumn; // Adjusted to use BigInteger if needed
    @FXML
    ...
    protected void onStableToggleClick() {
        // If toggled, show items with stable 5-min average
    ...
    private void loadDataWithSpinnerFilter() {
        CompletableFuture.runAsync(() -> {
            ...
            List<Item> filteredItems = allItems.stream()
                    ...
                    .filter(item -> item.getProfit().compareTo(BigInteger.ZERO) > 0) // Exclude items with negative profit
                    ...
                    .sorted((item1, item2) -> item2.getProfit().compareTo(item1.getProfit()))
                    .toList();
